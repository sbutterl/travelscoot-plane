(() => {
  ...
  // Resize with devicePixelRatio for crispness
  function resize() {
  ...
  // World & camera (for flying)
  let camX = 0;
  let camY = 0;

  // Simple particles/stars to collect
  let stars = [];
  let score = 0;
  ...
  // Builder parts: wings, tail, propeller
  // These are drawn shapes (so no extra images needed), but you can replace with PNGs later if you want.
  const parts = [
  ...
  // Targets are relative to the scoot position on screen
  const targets = {
  ...
  // Scoot/plane entity (in build mode it sits centered)
  const craft = {
    x: 0, y: 0,
    vx: 0, vy: 0,
    angle: 0,
  ...
  function layoutBuild() {
    // Place craft roughly center-left for better space for parts
    ...
    // Parts start near bottom area above the footer (but inside canvas)
    const baseY = h * 0.78;
    const gap = 14;
  ...
  function onDown(e) {
    ...
    // Pick top-most unplaced part under pointer
  ...
  function drawPart(p) {
    // Simple cartoony shapes per part id
    ctx.save();
  ...
  function drawScootAndPlane() {
    ...
    // During fly, tilt a bit
    if (state === State.FLY) ctx.rotate(craft.angle);

    // Transform effect: fade scoot → stylised plane overlay
    ...
    // Plane body overlay (cartoon), gradually appears
    ctx.globalAlpha = t;
  ...
  function drawStars() {
    ...
    // draw in world coords relative to camera
  ...
  function drawBackground() {
    // Simple moving clouds in fly mode
    ctx.save();
    ctx.globalAlpha = 0.7;
    for (let i=0;i<12;i++){
  ...
  function update(dt) {
    if (state === State.BUILD) {
      // keep layout stable
      // (if resized, layoutBuild handles)
    }
    ...
    if (state === State.TRANSFORM) {
      ...
      if (craft.transformT >= 1) {
        ...
        statusEl.textContent = "Fly! Collect stars ✨";
        spawnStars();
      }
    }
    ...
    if (state === State.FLY) {
      ...
      // damping
      craft.vx *= Math.pow(0.12, dt); // strong-ish damping
  ...
  function render() {
    ...
    if (state === State.FLY) drawBackground();

    // Targets (builder snap circles)
    drawTargets();

    // Craft (photo + transform overlay)
    if (state === State.FLY) {
      // keep craft drawn at center in fly mode (camera moves)
    ...
    if (state === State.BUILD) {
      ...
      // Draw placed parts attached during/after transform
      // (they remain visible as “upgrades”)
      for (const p of parts) {
        if (!p.placed) continue;
        // In fly mode they should sit on the plane center
        const t = targets[p.id];
    ...
    // Big prompt if scoot image missing
    if (!scootReady) {
      ctx.save();
  ...
  // Keep layout correct after image loads and after resizes
